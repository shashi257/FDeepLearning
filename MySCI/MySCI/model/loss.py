...
class LossFunction(nn.Module):
    ...
    def forward(self, input, illu):                                 # 低光观测和光照分量之间的损失
        Fidelity_Loss = self.l2_loss(illu, input)                   # 逼真度损失
        Smooth_Loss = self.smooth_loss(input, illu)                 # 平滑损失
...
class SmoothLoss(nn.Module):
    ...
    def rgb2yCbCr(self, input_im):
        ...
        # contiguous()，把tensor变成在内存中连续分布的形式,view使用的前提是variable是contiguous的，变成三列
        # 例如  一个长度的20向量x；x.view(-1, 4)等价于x.view(5, 4)

        """
        RGB转换为YCbCr
        # 获取亮度,即原图的灰度拷贝,绿色分量   这里的绿色分量有个值不同，和图像偏绿色是否有关系
    ...
    # input:低光观测        output:光照分量
    # output: output      input:input
    def forward(self, input, output):
        self.output = output
        self.input = self.rgb2yCbCr(input)
        sigma_color = -1.0 / (2 * self.sigma * self.sigma)       # 0.5X^2  |x|<1
